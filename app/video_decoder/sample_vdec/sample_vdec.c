...
else if (stContext.mVDecAttr.mType == PT_H264)
{
    ...
    if (fp_bs == NULL) {
        aloge("fopen fail! BitStreamFile: %p, %s", fp_bs, stContext.mConfigPara.mH264VbsPath);
        goto _exit_1;
    }

    // 创建一个用于存储解码后 YUV 数据的输出文件
    ...
    while ((bytes_read = fread(buffer, 1, buffer_size, fp_bs)) > 0) {
        int i = 0;
        while (i < bytes_read - 4) {
            // 检查起始码
            if (buffer[i] == 0x00 && buffer[i+1] == 0x00 && buffer[i+2] == 0x01) {
                start_code_prefix_len = 3;
            } else if (buffer[i] == 0x00 && buffer[i+1] == 0x00 && buffer[i+2] == 0x00 && buffer[i+3] == 0x01) {
            ...
            i += start_code_prefix_len;
        }

        if (frame_start > 0) {
            // 找到一帧图像的起始位置，读取该帧
            fseek(fp_bs, frame_start, SEEK_SET);
            bytes_read = fread(buffer, 1, buffer_size, fp_bs);
            if (bytes_read <= 0) {
                alogw("read bitstream fail or end of file, exit loop!");
                break;
            }

            nStreamInfo.pAddr = buffer;
            nStreamInfo.mLen = bytes_read;
            nStreamInfo.mbEndOfFrame = TRUE;
            alogd("read vbs packet! rd_cnt=%d", bytes_read);
